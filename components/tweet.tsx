  ...
  return (
    <div key={tweet._id} className="flex flex-col space-x-3 border-y p-5 border-gray-100">
      <div className="flex space-x-3">
        <img
          className="w-10 h-10 border rounded-full object-center"
      ...
      {commentBoxVisible && (
        <form onSubmit={handleSubmit} className="mt-3 space-x-3 flex">
          ...
          <button
            type="submit"
            disabled={!input}
            className="text-gwitter disabled:text-gray-200 "
          >
            Post
          </button>
        </form>
        ...
        <div className="my-2 mt-5 max-h-44 space-y-5 overflow-y-scroll border-t border-grey-100 p-5">
          {comments.map((comment) => (
            <div key={comment._id} className="relative flex space-x-2">
              <hr className="absolute left-6 top-10 h-8 border-x border-gwitter/30"></hr>
              <img
                className="mt-2 h-8 w-8 object-cover rounded-full border"
